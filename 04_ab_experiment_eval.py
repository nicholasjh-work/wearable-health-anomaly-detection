...
if "anomaly_score_v1" not in silver.columns:
    print("Anomaly scores missing, rebuilding...")
    X_a = silver[ANOMALY_FEATURES].fillna(0)
    sc = StandardScaler()
    X_s = sc.fit_transform(X_a)
    def _gt(df):
        flags = pd.Series(0, index=df.index)
        for l in df["activity_label"].unique():
            m = df["activity_label"]==l
            s = df.loc[m,"strain_score"]
            q1,q3 = s.quantile(0.05),s.quantile(0.95)
            iqr = q3-q1
            flags[m & ((df["strain_score"]<q1-2*iqr)|(df["strain_score"]>q3+2*iqr))] = 1
        return flags
    silver["anomaly_ground_truth"] = _gt(silver)
    gt = silver["anomaly_ground_truth"]
    gt_rate = gt.mean()
    iso1 = IsolationForest(n_estimators=200, contamination=0.05, random_state=42, n_jobs=-1)
    p1 = iso1.fit_predict(X_s)
    silver["anomaly_v1"] = np.where(p1==-1,1,0)
    silver["anomaly_score_v1"] = -iso1.score_samples(X_s)
    tc = min(gt_rate*1.5, 0.15)
    iso2 = IsolationForest(n_estimators=300, contamination=tc, random_state=42, n_jobs=-1)
    p2 = iso2.fit_predict(X_s)
    silver["anomaly_v2"] = np.where(p2==-1,1,0)
    silver["anomaly_score_v2"] = -iso2.score_samples(X_s)
else:
    gt = silver["anomaly_ground_truth"]

p_v1 = precision_score(gt, silver["anomaly_v1"], zero_division=0)
r_v1 = recall_score(gt, silver["anomaly_v1"], zero_division=0)
f1_1 = f1_score(gt, silver["anomaly_v1"], zero_division=0)
p_v2 = precision_score(gt, silver["anomaly_v2"], zero_division=0)
r_v2 = recall_score(gt, silver["anomaly_v2"], zero_division=0)
f1_2 = f1_score(gt, silver["anomaly_v2"], zero_division=0)

print(f"v1: precision={p_v1:.4f}, recall={r_v1:.4f}, f1={f1_1:.4f}")
print(f"v2: precision={p_v2:.4f}, recall={r_v2:.4f}, f1={f1_2:.4f}")

# COMMAND ----------

t_stat, p_value = stats.ttest_rel(silver["anomaly_score_v1"], silver["anomaly_score_v2"])
cohens_d = (silver["anomaly_score_v1"].mean() - silver["anomaly_score_v2"].mean()) / np.sqrt((silver["anomaly_score_v1"].std()**2 + silver["anomaly_score_v2"].std()**2) / 2)

f1_improvement = f1_2 - f1_1
sig = p_value < 0.05
effect = abs(cohens_d) >= 0.2
meets_prec = p_v2 >= 0.3

if sig and effect and meets_prec and f1_improvement > 0:
    decision = "SHIP"
elif sig and f1_improvement > 0:
    decision = "ITERATE"
else:
    decision = "KILL"

print(f"\n=== A/B Experiment Evaluation ===")
print(f"  F1 improvement: {f1_improvement*100:.2f}pp")
print(f"  p-value: {p_value:.6f}")
print(f"  Cohen's d: {cohens_d:.4f}")
print(f"  Significant: {sig}")
print(f"  Meaningful effect: {effect}")
